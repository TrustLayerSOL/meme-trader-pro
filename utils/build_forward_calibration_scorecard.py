...
from wallets.forward_calibration_scorecard import build_forward_calibration_scorecard  # noqa: E402


DEFAULT_RECORDS = ROOT / "data" / "reports" / "forward_testing" / "forward_outcome_records.jsonl"
DEFAULT_REPORT = ROOT / "data" / "reports" / "forward_testing" / "forward_calibration_scorecard.json"
DEFAULT_MARKDOWN = ROOT / "data" / "reports" / "forward_testing" / "forward_calibration_scorecard.md"
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# MemeTraderPro Forward Calibration Scorecard",
        "",
        "Review-only wallet calibration from captured forward outcomes.",
        "",
        "## Summary",
        "",
...
def write_forward_calibration_scorecard(
    *,
    ...
    report = build_forward_calibration_scorecard(read_jsonl(records_path), generated_at=generated_at)
    report["input_paths"] = {"forward_outcome_records": relative_path(records_path)}
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the read-only forward wallet calibration scorecard.")
