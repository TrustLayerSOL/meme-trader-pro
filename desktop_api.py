...
def build_wallet_ecosystem_intelligence_payload(state=None):
    state = state or read_state_files()
    existing = state.get("wallet_ecosystem_intelligence")
    if isinstance(existing, dict) and existing.get("mode") == "WALLET_ECOSYSTEM_INTELLIGENCE_STAGE5_REVIEW_ONLY":
        return {
            **existing,
            "read_only": True,
            "review_only": True,
            ...
            "wallet_list_mutated": False,
            "auto_trust_mutation_allowed": False,
            "source": "wallet_ecosystem_intelligence_json",
            "source_detail": str(WALLET_ECOSYSTEM_INTELLIGENCE_REPORT_FILE.relative_to(ROOT)),
        }
    report = build_wallet_ecosystem_intelligence_report(
        replay_scorecard=state.get("wallet_replay_scorecard") if isinstance(state.get("wallet_replay_scorecard"), dict) else {},
    )
    return {
        **report,
        "read_only": True,
        "source": "wallet_ecosystem_intelligence_computed",
        "source_detail": str(WALLET_ECOSYSTEM_INTELLIGENCE_REPORT_FILE.relative_to(ROOT)),
    }
...
def build_score_ready_market_context_payload(state=None):
    ...
    if isinstance(existing, dict) and existing.get("mode") == "SCORE_READY_MARKET_CONTEXT_REVIEW_ONLY":
        rows = existing.get("records") if isinstance(existing.get("records"), list) else []
        return {
            **existing,
            "read_only": True,
            "review_only": True,
            ...
            "wallet_list_mutated": False,
            "auto_trust_mutation_allowed": False,
            "source": "score_ready_market_context_json",
            "source_detail": str(SCORE_READY_MARKET_CONTEXT_REPORT_FILE.relative_to(ROOT)),
            "count": len(rows),
        }
    report = build_score_ready_market_context_report(
        onchain_market_context_records=state.get("onchain_market_context", {}).get("records", []) if isinstance(state.get("onchain_market_context"), dict) else [],
        supply_evidence_records=state.get("onchain_supply_evidence", {}).get("records", []) if isinstance(state.get("onchain_supply_evidence"), dict) else [],
    )
    return {
        **report,
        "read_only": True,
        "source": "score_ready_market_context_computed",
        "source_detail": str(SCORE_READY_MARKET_CONTEXT_REPORT_FILE.relative_to(ROOT)),
        "count": len(report.get("records") if isinstance(report.get("records"), list) else []),
    }
...
def build_archival_supply_evidence_payload(state=None):
    state = state or read_state_files()
    existing = state.get("archival_supply_evidence")
    if isinstance(existing, dict) and existing.get("mode") == "ARCHIVAL_SUPPLY_EVIDENCE_REVIEW_ONLY":
        rows = existing.get("records") if isinstance(existing.get("records"), list) else []
        return {
            **existing,
            "read_only": True,
            ...
            "auto_trust_mutation_allowed": False,
            "wallet_trust_mutation_allowed": False,
            "source": "archival_supply_evidence_json",
            "source_detail": str(ARCHIVAL_SUPPLY_EVIDENCE_REPORT_FILE.relative_to(ROOT)),
            "count": len(rows),
        }
    ...
    return {
        **report,
        "read_only": True,
        "source": "archival_supply_evidence_computed_without_snapshots",
        "source_detail": str(ARCHIVAL_SUPPLY_EVIDENCE_REPORT_FILE.relative_to(ROOT)),
        "count": len(report.get("records") if isinstance(report.get("records"), list) else []),
    }


def build_archival_mint_snapshot_collection_payload(state=None):
    state = state or read_state_files()
    existing = state.get("archival_mint_snapshot_collection")
    if isinstance(existing, dict) and existing.get("mode") == "ARCHIVAL_MINT_SUPPLY_SNAPSHOT_COLLECTION_REVIEW_ONLY":
        rows = existing.get("requests") if isinstance(existing.get("requests"), list) else []
        return {
            **existing,
            "read_only": True,
            "review_only": True,
            ...
            "auto_trust_mutation_allowed": False,
            "wallet_trust_mutation_allowed": False,
            "source": "archival_mint_snapshot_collection_json",
            "source_detail": str(ARCHIVAL_MINT_SNAPSHOT_COLLECTION_REPORT_FILE.relative_to(ROOT)),
            "count": len(rows),
        }
    plan = state.get("archival_supply_recovery_plan") if isinstance(state.get("archival_supply_recovery_plan"), dict) else {}
    ...
    return {
        **report,
        "read_only": True,
        "source": "archival_mint_snapshot_collection_computed_dry_run",
        "source_detail": str(ARCHIVAL_MINT_SNAPSHOT_COLLECTION_REPORT_FILE.relative_to(ROOT)),
        "count": len(report.get("requests") if isinstance(report.get("requests"), list) else []),
    }
...
def build_archival_mint_history_collection_payload(state=None):
    state = state or read_state_files()
    existing = state.get("archival_mint_history_collection")
    if isinstance(existing, dict) and existing.get("mode") == "ARCHIVAL_MINT_HISTORY_COLLECTION_REVIEW_ONLY":
        rows = existing.get("targets") if isinstance(existing.get("targets"), list) else []
        return {
            **existing,
            "read_only": True,
            ...
            "auto_trust_mutation_allowed": False,
            "wallet_trust_mutation_allowed": False,
            "source": "archival_mint_history_collection_json",
            "source_detail": str(ARCHIVAL_MINT_HISTORY_COLLECTION_REPORT_FILE.relative_to(ROOT)),
            "count": len(rows),
        }
    plan = state.get("archival_supply_recovery_plan") if isinstance(state.get("archival_supply_recovery_plan"), dict) else {}
    ...
    return {
        **report,
        "read_only": True,
        "source": "archival_mint_history_collection_computed_dry_run",
        "source_detail": str(ARCHIVAL_MINT_HISTORY_COLLECTION_REPORT_FILE.relative_to(ROOT)),
        "count": len(report.get("targets") if isinstance(report.get("targets"), list) else []),
    }
...
def build_archival_mint_pagination_plan_payload(state=None):
    state = state or read_state_files()
    existing = state.get("archival_mint_pagination_plan")
    if isinstance(existing, dict) and existing.get("mode") == "ARCHIVAL_MINT_PAGINATION_PLAN_REVIEW_ONLY":
        rows = existing.get("rows") if isinstance(existing.get("rows"), list) else []
        return {
            **existing,
            "read_only": True,
            ...
            "auto_trust_mutation_allowed": False,
            "wallet_trust_mutation_allowed": False,
            "source": "archival_mint_pagination_plan_json",
            "source_detail": str(ARCHIVAL_MINT_PAGINATION_PLAN_REPORT_FILE.relative_to(ROOT)),
            "count": len(rows),
        }
    progress = state.get("archival_mint_history_progress") if isinstance(state.get("archival_mint_history_progress"), dict) else {}
    report = build_archival_mint_pagination_plan_report(progress_report=progress)
    return {
        **report,
        "read_only": True,
        "source": "archival_mint_pagination_plan_computed",
        "source_detail": str(ARCHIVAL_MINT_PAGINATION_PLAN_REPORT_FILE.relative_to(ROOT)),
        "count": len(report.get("rows") if isinstance(report.get("rows"), list) else []),
    }
...
def build_archival_account_state_provider_probe_payload(state=None):
    state = state or read_state_files()
    existing = state.get("archival_account_state_provider_probe")
    if isinstance(existing, dict) and existing.get("mode") == "ARCHIVAL_ACCOUNT_STATE_PROVIDER_PROBE_REVIEW_ONLY":
        return {
            **existing,
            "read_only": True,
            "review_only": True,
            ...
            "source": "archival_account_state_provider_probe_json",
            "source_detail": str(ARCHIVAL_ACCOUNT_STATE_PROVIDER_PROBE_REPORT_FILE.relative_to(ROOT)),
        }
    pagination_plan = state.get("archival_mint_pagination_plan") if isinstance(state.get("archival_mint_pagination_plan"), dict) else {}
    provider_evaluation = (
        state.get("archival_account_state_provider_evaluation")
        if isinstance(state.get("archival_account_state_provider_evaluation"), dict)
        else {}
    )
    ...
    return {
        **report,
        "read_only": True,
        "source": "archival_account_state_provider_probe_computed",
        "source_detail": str(ARCHIVAL_ACCOUNT_STATE_PROVIDER_PROBE_REPORT_FILE.relative_to(ROOT)),
    }


def build_archival_account_state_provider_probe_request_payload(state=None):
    state = state or read_state_files()
    existing = state.get("archival_account_state_provider_probe_request")
    if isinstance(existing, dict) and existing.get("mode") == "ARCHIVAL_ACCOUNT_STATE_PROVIDER_REQUEST_BUNDLE_REVIEW_ONLY":
        return {
            **existing,
            "read_only": True,
            "review_only": True,
            ...
            "wallet_trust_mutation_allowed": False,
            "supply_snapshot_import_allowed": False,
            "source": "archival_account_state_provider_probe_request_json",
            "source_detail": str(ARCHIVAL_ACCOUNT_STATE_PROVIDER_PROBE_REQUEST_REPORT_FILE.relative_to(ROOT)),
        }
    provider_probe = (
        state.get("archival_account_state_provider_probe")
        if isinstance(state.get("archival_account_state_provider_probe"), dict)
        else {}
    )
    report = build_archival_account_state_provider_probe_request_report(provider_probe=provider_probe)
    return {
        **report,
        "read_only": True,
        "source": "archival_account_state_provider_probe_request_computed",
        "source_detail": str(ARCHIVAL_ACCOUNT_STATE_PROVIDER_PROBE_REQUEST_REPORT_FILE.relative_to(ROOT)),
    }
...
def route_request(method, raw_path, body=None, headers=None):
    ...
    if path == "/api/archival-supply-evidence":
    ...
    if path == "/api/archival-account-state-provider-probe":
