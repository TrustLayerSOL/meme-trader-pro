...
def test_cli_requires_dry_run(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            ...
            "-m",
            "research.mtp_research.validation.run_migration_graduation_enrichment_planner",
            "--candidates-path",
            str(candidates_path),
            "--output-dir",
...
def test_cli_writes_dry_run_outputs_without_network_calls(tmp_path: Path) -> None:
    ...
    assert (output_dir / "migration_graduation_enrichment_dry_run_plan.json").exists()
    assert (output_dir / "migration_graduation_enrichment_dry_run_plan.md").exists()
