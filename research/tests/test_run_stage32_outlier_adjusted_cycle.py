...
from research.mtp_research.validation.run_stage32_outlier_adjusted_cycle import main


def test_run_stage32_cycle_cli_runs_offline(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    NormalizedEventStore(events_path).upsert_many(
        [
            ...
            for idx, (ts, price) in enumerate([(100, 1.0), (200, 1.5), (500, 2.0), (1000, 2.2)])
        ]
    )
    monkeypatch.setattr(
        "sys.argv",
        [
            "run_stage32_outlier_adjusted_cycle",
            "--dataset-path",
            str(dataset_path),
            "--events-path",
            str(events_path),
