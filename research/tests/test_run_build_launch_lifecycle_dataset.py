...
def test_lifecycle_dataset_cli_can_include_all_verified_census_launches(tmp_path: Path) -> None:
    launch_ts = int(datetime(2026, 6, 1, 14, 30, tzinfo=PACIFIC).timestamp())
    census_path = tmp_path / "census.jsonl"
    events_path = tmp_path / "events.jsonl"
    output_dir = tmp_path / "launch_regime"
    write_creation_census(
    ...
    result = subprocess.run(
        [
            ...
            "--target-launches",
            "1",
            "--include-outside-configured-regime",
        ],
        cwd=Path(__file__).resolve().parents[2],
        check=True,
        capture_output=True,
    ...
    assert "launches_collected=1" in result.stdout
    assert "outside_configured_regime" in result.stdout
