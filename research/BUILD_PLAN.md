...
- [~] Wallet candidate audit. `wallets/wallet_candidate_audit.py` and `utils/build_wallet_candidate_audit.py` generate `data/wallet_candidate_audit.json` for snapshot-linked and replay-review promotion/demotion candidates. Current active queue has `0` demotion-review candidates, `0` active promotion-review candidates, and resolved applied replay/paper-watch actions. It is review-only and blocks wallet-list apply unless an explicit approved decision maps cleanly to current audit evidence.
- [~] Historical replay dataset contract. `research/historical_replay_dataset.py` and `utils/build_historical_replay_dataset.py` create `data/historical_replay/replay_events.jsonl` from accepted trades, rejected signals, wallet observations, and later outcome labels while keeping future/outcome data outside decision context. Events now declare fixed `30s`, `2m`, `5m`, and `15m` evaluation windows plus realistic slippage, latency, liquidity-floor, and failed-fill assumptions. Current local summary has `6,041` events, `0` unsafe/leakage flags, `726` fillable-with-assumptions events, `667` liquidity-floor failures, and `4,648` unknown-liquidity events after decision-time prior-snapshot enrichment. Window outcome counts now exist for each fixed horizon; `15m` currently shows `76` runner, `15` rug, `149` dead, `275` loser, and `5,526` unknown labels. This is review-only and must not feed live execution.
- [~] Wallet replay scorecard. `wallets/wallet_replay_scorecard.py` and `utils/build_wallet_replay_scorecard.py` generate `data/wallet_replay_scorecard.json` from historical replay events. The scorecard ranks wallets by fixed-window outcome coverage/performance, separates coverage from outcome quality, surfaces fillability, tracks market-regime exposure, and exposes repeated co-entry partners/pairs. `wallets/wallet_replay_review.py`, `/api/wallet-replay-review`, the Obsidian `Wallet Replay Ecosystem Review` dashboard, and `utils/sync_replay_wallet_decisions.py` now turn that scorecard into a read-only review queue with conservative machine recommendations that can be saved into `data/wallet_review_decisions.json`. Replay recommendations can now flow through `data/wallet_candidate_audit.json` and the guarded wallet-list apply tool. Current local scorecard covers `364` wallets across `6,041` replay events and reports `50` top co-entry pairs.
...
- [~] Read-only wallet-history backfill. `wallets/wallet_history_backfill.py`, `wallets/wallet_history_parser.py`, `wallets/wallet_evidence_models.py`, `wallets/wallet_evidence_reporter.py`, and `utils/run_wallet_history_backfill.py` process `COLLECT_WALLET_HISTORY` candidate targets into structured wallet evidence records. The first bounded run processed `45` wallets, produced `146` evidence rows, partially backfilled `41`, blocked `4` for missing token-delta evidence, preserved `180` raw transactions in a versioned local file, exposed `/api/wallet-history-backfill`, and rendered `MemeTraderPro/Dashboards/Wallet History Backfill.md`. It remains review-only.
- [~] Wallet evidence enrichment. `wallets/wallet_evidence_enrichment.py` and `utils/enrich_wallet_history_evidence.py` enrich wallet-history evidence with prior-only decision-time market context from SQLite `token_snapshots`/`swap_ticks` plus separate later outcome windows. Current local run processed `520` evidence rows, added entry context to `151`, fully enriched `16`, marked `369` missing market context, exposed `/api/wallet-evidence-enrichment`, and rendered `MemeTraderPro/Dashboards/Wallet Evidence Enrichment.md`. It remains review-only and cannot promote, demote, or trade.
- [~] Missing market-context targets. `wallets/wallet_missing_market_context.py` and `utils/build_wallet_missing_market_context_targets.py` generate `data/wallet_backfills/wallet_missing_market_context_report.json` from enriched wallet evidence. Current local queue has `36` target mints, `194` deduped missing-context evidence rows, and `33` affected wallets after excluding quote mints. It exposes `/api/wallet-missing-market-context` and renders `MemeTraderPro/Dashboards/Wallet Missing Market Context.md`. It remains review-only.
...
- [~] Historical market-context backfill checkpoint. `wallets/historical_market_context_backfill.py` and `utils/backfill_historical_market_context.py` classify the remaining missing evidence against preserved raw transactions. Current local run scanned `194` missing-context rows, partially recovered `47` rows from paired token/quote raw transaction deltas, blocked `100` rows for missing raw transactions, and blocked `47` rows for missing usable quote-price deltas. This is a 50% lane checkpoint: it proves the classification/recovery path and preserves block reasons, but it does not yet create trusted USD price/liquidity snapshots or restore lost `swap_ticks`.
...
- Use `data/reports/historical_backfill/historical_market_context_backfill_report.json` to separate partial transaction-derived context from rows that need raw transaction recovery or trusted historical market snapshots.
