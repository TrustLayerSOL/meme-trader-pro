...
<mark>Current focus: Build the forward evidence loop so current wallet activity, decision-time context, and later outcomes are captured going forward instead of relying primarily on expensive or partial historical reconstruction. The current blocker is operator review of the single forward-signal rollup through the Obsidian dashboard; the artifact is intentionally `manual_review_required`, not promotion.</mark>
...
| Stage 8 | Replay Validation + Forward Testing | Complete as a validation-loop contract. The replay summary, Stage 6 realism gate, wallet replay scorecard, wallet outcome ledger, candidate backfill queue, and forward calibration reports now cross-check each other. Proof readiness remains `12%` because the top-level trust gate is still based on historical replay proof, not forward-only current observations. Historical known 15m outcome density is `710` / `6,042` (`12%`), and fillability evidence is `81%` vs `70%` required. Positive fills remain `52%`; the extra `29%` is useful failed-liquidity evidence, not successful execution. Forward calibration is visible separately: current local forward reports show `3,551` rows, `1,070` known 15m outcomes, `2,538` blocked missing-entry-context rows, `6` pending windows, `0` trust/list mutations, and `5` observed days. The forward wallet calibration scorecard now ranks `31` wallets: `1` review-behavioral-signal wallet, `0` risk-review wallets, `7` flat/noise candidates, `22` missing-context wallets, and `1` collect-more-evidence wallet. The conservative forward recommendation layer therefore allows `0` promotions, holds flat-only wallets out of promotion review, routes `22` wallets to entry-context repair, and routes `1` wallet to more evidence. Market-context and archival supply coverage remain the dominant blockers. The forward wallet activity collector now has optional current market-context capture, fixed-window outcome resolution, daily calibration reports, a no-paid-RPC default, public fallback URL support, user-agent headers for public gateways, public-RPC preflight, adaptive degraded-provider throttling, and a small free-RPC canary runner. Helius/paid RPC now requires the explicit `--allow-paid-rpc` opt-in; default runs use public/free RPC only and must remain small/throttled because free endpoints rate-limit. The latest smoke canary processed `5` wallets, created `2` evidence rows, captured `2` market snapshots, projected `4,320` RPC/day, and still recommended `FREE_RPC_UNSTABLE` because per-wallet RPC errors remained high. | 100% |
...
- <mark>Forward proof-data calibration after Stage 9. Historical/manual recovery remains available, but the main path is now preserving current wallet activity safely, capturing collection-time market context, resolving later outcomes, and making those outcomes visible beside Stage 8 proof readiness without treating them as historical proof. Current forward calibration shows `1,080` known 15m outcomes across `3,581` forward rows, all review-only with `0` trust/list mutations. The wallet-level forward scorecard finds `59` runner 15m outcomes, `1,021` flat 15m outcomes, `1` review-behavioral-signal wallet, `7` flat/noise candidate wallets, `23` wallets mostly blocked by missing entry context, and `1` collect-more-evidence wallet. The conservative recommendation layer allows `0` promotions from this forward batch. The forward collector, market-context capture, fixed-window resolver, daily calibration report, scorecard, recommendations, public-RPC preflight, adaptive degraded-provider throttling, free-RPC canary runner, provider-rotation report, and disabled recurring-schedule proposal are wired behind a no-paid-RPC default: current runs use public/free Solana RPC unless the operator explicitly passes `--allow-paid-rpc`. Public/free mode supports explicit fallback URLs; the provider rotation found `2` healthy public endpoints, and the three latest bounded free-RPC canaries all recommended `FREE_RPC_USABLE_SMALL_THROTTLED`. A conservative recurring proposal is now ready for review but remains disabled. Stage 10 - Semi-Autonomous Risk Engine remains 0% and deferred.</mark>
...
- Current local report state: `behavioral_trust_justified=false`, `trust_ready_patterns=0`, `proof_readiness_pct=12`, `wallet_score_readiness_pct=19`, `score_ready_market_context_records=124`, `supply_recovered_records=83`, and `508` near-score-ready rows still blocked on missing archival supply or market-cap proof.
...
- Proof-readiness blocker reduction now ranks the exact next queue at `/api/proof-readiness-blocker-reduction`: `124` score-ready market-context records, `710` historical known 15m outcomes vs `30` required, `81%` fillability evidence coverage vs `70%` required, `52%` positive fill rate, and `643` historical wallet-evidence context rows still needing final supply/market-cap proof. Stage 8 also exposes forward calibration separately with `1,009` forward known 15m outcomes across `3,547` rows, but those forward rows do not raise historical proof readiness or unlock trust. This is a queue, not trust permission.
...
- Archival supply recovery planning turns near-score-ready rows into `/api/archival-supply-recovery-plan` and request bundles. Current score-ready classification leaves `508` rows needing archival supply / market-cap proof, with `0` wallet trust/list mutations. This is an evidence shopping list only; it does not fetch supply, infer supply, substitute current supply, or change trust.
...
- Focused manual supply research now gives a no-paid-provider path for the same `349` request rows. `python3 main.py export-focused-supply-research --limit 349` writes a manual research packet and supply template under `data/manual_research/`. `python3 main.py import-focused-supply-evidence ...` stores manual evidence separately and emits replay-safe supply snapshots only for `A_FULL_REPLAY_SAFE` rows whose response slot is at or before the request boundary. `utils/build_archival_supply_evidence.py` now consumes those Tier A focused manual snapshots along with provider, reconstruction, and stability-proven snapshots.
...
- Post-decision supply transaction collection now writes `data/reports/historical_backfill/post_decision_supply_transaction_collection_report.json`. It is the no-paid path for turning current supply snapshots into replay-compatible evidence without guessing: it refreshes mint-account signatures from the chain head, requires overlap with the existing checkpoint or a decision-slot boundary, preserves missing post-decision transaction bodies, and leaves proof decisions to the coverage/stability reports. It cannot mutate wallet trust, wallet lists, or execution.
- Post-decision supply coverage now writes `data/reports/historical_backfill/post_decision_supply_coverage_report.json` and `post_decision_supply_completeness_updates.json`. Current local run marks `0` stable tokens because the existing signature checkpoint does not yet carry a collection timestamp that can prove it was collected after the current supply snapshot. The new transaction-body collector is the next bounded route to create that timestamped checkpoint and transaction-body coverage.
- Supply stability evidence now writes `data/reports/historical_backfill/supply_stability_evidence_report.json` and `supply_stability_evidence_snapshots.jsonl`. The stricter current local run proves `0` stable-current-supply tokens and blocks all `75` token requirements: `64` still lack complete decision-slot mint history, and `11` reconstructed-history tokens still lack explicit post-decision coverage before current supply can be reused. This prevents current supply from being treated as historical proof.
...
- Stage 10 is intentionally deferred because proof readiness is still `12%`. The next practical implementation step is not risk automation; it is archival supply / market-cap recovery for the near-score-ready rows that now have price and liquidity but still lack decision-time supply or direct market-cap proof.
...
- [x] Score-ready market-context classifier. `wallets/score_ready_market_context.py` and `utils/build_score_ready_market_context.py` generate `data/reports/historical_backfill/score_ready_market_context_report.json` and `data/reports/historical_backfill/score_ready_market_context_records.jsonl` from on-chain market-context recovery, archival supply evidence records, and Tier A manual historical market-cap evidence. Current local report scans `643` rows, marks `124` score-ready after replay-safe supply reconstruction, identifies `508` near-score-ready archival-supply / market-cap candidates across `61` tokens and `29` wallets, and keeps `9` price-missing rows plus `2` liquidity-missing rows blocked. `/api/score-ready-market-context` exposes it as read-only. This narrows the next proof-readiness work without inventing supply, market cap, or wallet trust.
...
- [x] Focused manual supply research lane. `wallets/focused_manual_supply_research.py`, `utils/build_focused_manual_supply_research.py`, `python3 main.py export-focused-supply-research`, and `python3 main.py import-focused-supply-evidence` provide a free/manual path for the focused `349` provider-recommended requests. Current local export writes `349` request rows across `43` tokens to `data/manual_research/focused_manual_supply_research_packet.csv` and `data/manual_research/focused_manual_supply_template.csv`. Only Tier A rows can emit `data/manual_research/focused_manual_supply_snapshots.jsonl`, and the archival supply evidence builder consumes that file by default. This does not fabricate supply or move proof-readiness without operator-supplied evidence.
...
- [x] Post-decision supply coverage gate. `wallets/post_decision_supply_coverage.py` and `utils/build_post_decision_supply_coverage.py` generate `data/reports/historical_backfill/post_decision_supply_coverage_report.json` and `data/reports/historical_backfill/post_decision_supply_completeness_updates.json`. Current local run marks `0` stable tokens because the existing signature checkpoint lacks a collection timestamp; future checkpoint runs now preserve that timestamp so post-decision transaction-body coverage can be audited instead of assumed.
- [x] Supply stability evidence gate. `wallets/supply_stability_evidence.py` and `utils/build_supply_stability_evidence.py` generate `data/reports/historical_backfill/supply_stability_evidence_report.json` and `data/reports/historical_backfill/supply_stability_evidence_snapshots.jsonl`. Current local run proves `0` stable-current-supply tokens and blocks all `75` token requirements until both decision-slot history and post-decision history are explicitly proven. This gate allows current supply only when it is proven equivalent to decision-time supply.
- [x] Archival supply evidence import. `wallets/archival_supply_evidence.py` and `utils/build_archival_supply_evidence.py` generate `data/reports/historical_backfill/archival_supply_evidence_report.json` and `data/reports/historical_backfill/archival_supply_evidence_records.jsonl` from the archival supply recovery plan plus optional historical mint-account snapshots, local reconstruction snapshots, and stability-proven current-supply snapshots. Current local report scans `591` candidate rows, recovers `60` replay-safe supply rows across `11` tokens from complete mint/burn reconstruction, blocks `531` as `missing_archival_snapshot`, and affects `75` tokens / `33` wallets. `/api/archival-supply-evidence` exposes it as read-only. The score-ready classifier consumes recovered archival supply records when they exist and recomputes market cap from decision-time price times historical supply.
...
- [x] Stage 8 replay validation readiness gate. `research/replay_validation_readiness.py` and `utils/build_replay_validation_readiness.py` generate `data/reports/replay_validation/stage8_validation_readiness_report.json`. Current local report marks the Stage 8 validation contract at `100%` because the replay summary, Stage 6 realism gate, wallet replay scorecard, wallet outcome ledger, and candidate backfill queue are aligned, review-only, and live-execution locked. It now reports `12%` Stage 8 proof readiness with `710` known 15m outcomes, `81%` fillability evidence coverage, `52%` positive fill rate, `29%` failed-liquidity evidence, `19%` unknown-liquidity rows, and `19%` data score readiness. Failed-liquidity rows count as evidence because they prove a trade would not realistically fill; they do not count as positive fills.
- [x] Forward entry-context repair plan. `wallets/forward_entry_context_repair_plan.py` and `utils/build_forward_entry_context_repair_plan.py` generate `data/reports/forward_testing/forward_entry_context_repair_plan.json` / `.md`. Current local report covers `31` wallets and `2,538` blocked forward rows: `562` rows have quote-anchor repair evidence, but only `83` of those also have later market snapshots; `479` quote-anchor rows still lack snapshot support, and `1,976` rows need market snapshots entirely. The plan is review-only, keeps live execution locked, and allows `0` wallet-list or trust mutations.
- [x] Forward entry-context resolver. `wallets/forward_entry_context_resolver.py` and `utils/build_forward_entry_context_resolver.py` generate `data/reports/forward_testing/forward_entry_context_resolver_report.json` / `.md`, plus repaired and rejected row JSONL files. Current local resolver scanned `2,538` blocked rows, found `562` quote-anchor candidates, repaired `82` rows with at-or-after-signal market snapshots, produced `59` known 15m outcomes, and kept `2,456` rows blocked (`480` missing later market snapshots, `1,976` missing valid quote anchors). It is review-only and allows `0` wallet-list or trust mutations.
- [x] Merged forward calibration scorecard. `wallets/forward_merged_calibration_scorecard.py` and `utils/build_forward_merged_calibration_scorecard.py` generate `data/reports/forward_testing/forward_merged_calibration_scorecard.json` / `.md`. Current local merged scorecard keeps total forward records at `3,547` by replacing `82` original blocked rows with repaired rows by `event_id`. Known 15m outcomes rise from `1,009` to `1,068`, runner 15m outcomes rise to `59`, blocked rows drop to `2,456`, and `1` wallet moves into review-behavioral-signal status. This is analysis-only and allows `0` wallet-list or trust mutations.
- [x] Merged forward calibration recommendations. `wallets/forward_merged_calibration_recommendations.py` and `utils/build_forward_merged_calibration_recommendations.py` generate `data/reports/forward_testing/forward_merged_calibration_recommendations.json` / `.md`. Current local report flags `1` review-forward-signal wallet, `22` fix-context wallets, `7` flat-only hold wallets, and `1` collect-more wallet. Promotions remain `0`, wallet-list mutations remain `0`, and auto trust mutations remain `0`.
- [x] Forward signal review packet. `wallets/forward_signal_review_packet.py` and `utils/build_forward_signal_review_packet.py` generate `data/reports/forward_testing/forward_signal_review_packet.json` / `.csv` / `.md`. Current local packet covers `1` review-forward-signal wallet, `73` repaired review rows, `59` runner 15m rows, `14` unknown 15m rows, `0` flat 15m rows, `0` promotions allowed, `0` wallet-list mutations, and `0` auto trust mutations. This is a manual review artifact only; it cannot approve wallet trust or change the wallet list.
- [x] Forward signal review validator. `wallets/forward_signal_review_validator.py` and `utils/build_forward_signal_review_validator.py` generate `data/reports/forward_testing/forward_signal_review_validator.json` / `.csv` / `.md`. Current local validator reviews `1` wallet and marks `1` as `repeatability_supported_manual_review`, with `0` concentrated-anomaly wallets, `0` trust mutations allowed, and `0` wallet-list mutations allowed. This confirms the repaired runner evidence is broad enough for manual review, not trust approval.
- [x] Forward signal decision prep. `wallets/forward_signal_decision_prep.py` and `utils/build_forward_signal_decision_prep.py` generate `data/reports/forward_testing/forward_signal_decision_prep.json` / `.csv` / `.md`. Current local prep creates `1` `manual_review_required` decision for the repeatability-supported wallet, holds out `0` wallets, allows `0` promotions, allows `0` trust mutations, and allows `0` wallet-list mutations. This is the human-review handoff only.
- [x] Forward signal operator rollup. `wallets/forward_signal_operator_rollup.py` and `utils/build_forward_signal_operator_rollup.py` generate `data/reports/forward_testing/forward_signal_operator_rollup.json` / `.md`. Current local rollup combines packet, validator, and decision prep into one operator file: `1` wallet, `1` manual-review-required decision, `1` repeatability-supported wallet, `0` promotions allowed, `0` trust mutations allowed, and `0` wallet-list mutations allowed.
- [x] First forward wallet trust review packet. `wallets/forward_wallet_trust_review_packet.py` and `utils/build_forward_wallet_trust_review_packet.py` generate deterministic review-only exports under `data/reports/forward_testing/wallet_trust_review/`, plus Obsidian review pages when a vault is available. Current local packet covers `3` review-behavioral-signal wallets and `1,572` event evidence rows, with `2,627` context-blocked records, `6` wallet behavior buckets, RPC health `healthy_but_interval_mismatch`, safety status `locked_safe`, `0` promotions, `0` wallet-list mutations, and `0` auto trust mutations. The top wallet is a continued-validation candidate only; it remains not trusted.
- [x] Forward context blocker reduction packet. `wallets/forward_context_blocker_reduction.py` and `utils/build_forward_context_blocker_reduction.py` generate `data/reports/forward_testing/context_blocker_reduction/forward_context_blocker_reduction_<run_id>.json` / `.csv` / `.md`. Current local packet covers `21` fix-context wallets and `1,209` blocked records: `468` missing-later-market-snapshot rows and `741` missing-valid-execution-price-quote rows. It prioritizes `3` market-snapshot wallets and `18` entry-timestamp/quote wallets while keeping promotions, trust mutations, wallet-list mutations, and execution disabled.
- [x] Helius quote probe. `wallets/forward_helius_quote_probe.py`, `utils/run_forward_helius_quote_probe.py`, and native SOL quote recovery in `wallets/wallet_history_parser.py` generate bounded review-only exports under `data/reports/forward_testing/helius_quote_probe/`. Dry-run is the default; paid RPC requires explicit `--execute --allow-paid-rpc`. Current local probes fetched `25` / `25` Helius transaction bodies with `0` RPC failures and recovered `20` quote anchors using native SOL balance deltas with fee adjustment; `5` rows remain blocked because the tracked wallet had token movement but no quote movement. The probe writes `0` repaired rows, allows `0` promotions, and performs `0` wallet-list/trust mutations.
- [x] Helius quote-anchor merge. `wallets/forward_helius_quote_anchor_merge.py` and `utils/build_forward_helius_quote_anchor_merge.py` overlay recoverable Helius quote anchors onto a temporary forward record set and run the existing entry-context resolver without overwriting canonical resolver files. Current local merge used `25` quote-probe rows, augmented `20` records, produced `94` isolated resolver-ready rows, and reduced merged scorecard blocked records from `2,627` to `2,617` while keeping known 15m outcomes at `1,280`. It writes `0` canonical repair rows, allows `0` promotions, and performs `0` wallet-list/trust mutations.
- [x] Forward market snapshot repair queue. `wallets/forward_market_snapshot_repair_queue.py` and `utils/build_forward_market_snapshot_repair_queue.py` group quote-anchored rows that remain blocked by `missing_later_market_snapshot`. Current local queue from the 250-row Helius merge covers `614` missing-snapshot rows across `324` token mints and `17` wallets; the top token could unblock `18` rows if later snapshots are captured/imported. This queue writes no repaired rows, allows `0` promotions, and performs `0` wallet-list/trust mutations.
- [x] Forward market snapshot capture. `wallets/forward_market_snapshot_capture.py` and `utils/capture_forward_market_snapshot_repair_queue.py` run bounded review-only capture passes over the repair queue. The tool supports `--start-index` so later batches can skip already-tested queue rows while using the previous combined context file. Current local cumulative top-50 execution selected `50` queued mints across five batches, captured `11` usable current market snapshots, had `39` provider misses, and wrote isolated captured/combined context files. When the cumulative context file was routed through the isolated 250-row Helius quote-anchor merge, resolver-ready rows increased from `180` to `246`, blocked records decreased from `2,531` to `2,465`, known 15m outcomes stayed at `1,282`, and review-behavioral-signal wallets stayed at `4`. Batch 5 captured `0` usable snapshots and produced no scorecard improvement, so broad current-snapshot batching is paused pending smarter target selection. Canonical market context and resolver files were not overwritten; promotions, wallet-list mutations, trust mutations, and trades remained `0`.
- [x] Forward signal Obsidian review surface. `obsidian_export/intelligence_notes.py` and `obsidian_export/exporter.py` load `data/reports/forward_testing/forward_signal_operator_rollup.json` into the generated Obsidian dashboards as `Dashboards/Forward Signal Operator Review.md` and add a command-center queue link. This keeps the forward-signal wallet visible in the daily research review without approving promotion, wallet trust mutation, wallet-list mutation, or execution.
- [x] Forward enhanced-observation lane. `wallets/forward_enhanced_observation.py` and `utils/build_forward_enhanced_observation.py` generate `data/reports/forward_testing/forward_enhanced_observation_watchlist.json` / `.md` from the operator rollup. Current local watchlist places `1` repeatability-supported manual-review wallet into `enhanced_observation`, requires at least `25` next forward signals across at least `10` distinct next token mints before another trust discussion, and keeps promotion, trust mutation, wallet-list mutation, and execution mutation at `0`.
- [x] Forward enhanced-observation follow-up. `wallets/forward_enhanced_observation_followup.py` and `utils/build_forward_enhanced_observation_followup.py` generate `data/reports/forward_testing/forward_enhanced_observation_followup.json` / `.md`. It compares enhanced-observation wallets against only forward records after the watchlist baseline, avoiding double-counting the evidence that created the watchlist. Current local report tracks `1` wallet, finds `0` new forward records after baseline, keeps `1` wallet in `collect_more_forward_evidence`, and allows `0` promotions, trust mutations, wallet-list mutations, or execution mutations.
