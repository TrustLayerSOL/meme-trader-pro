...
| Stage 4 | Wallet Promotion/Demotion System | Review-only scorecard consumer now exists, feeds the guarded human review/apply evidence layer, is visible through the local API/Obsidian review surface, can prepare draft-only decision records, summarizes review buckets, and now converts non-actionable rows into an evidence collection plan. Stage 4 can classify wallets, expose the queue, draft non-approved proposed decisions, separate review buckets, and identify exact next evidence steps without auto-applying changes. Automatic trust evolution remains future work. | 75% |
...
- <mark>Stage 4 - Wallet Promotion/Demotion System: 75%</mark>
...
- Stage 3 is complete as a wallet evidence engine. Stage 4 now consumes the scorecard conservatively, feeds the human decision/apply guard through `data/wallet_candidate_audit.json`, exposes that review queue through `/api/wallet-candidate-audit`, prepares draft-only review decisions through `/api/wallet-candidate-decision-prep`, summarizes the remaining review buckets through `/api/wallet-candidate-review-summary`, and converts them into `/api/wallet-candidate-collection-plan`. Current local collection plan covers `49` targets: `36` need outcomes plus market context, `9` need outcome labels, `4` need manual risk review, `5` resolved rows are excluded, and `0` wallet-list changes are auto-applied. The next grounded step is a read-only batch runner for the outcome/market-context collection queue.
