...
| Stage 4 | Wallet Promotion/Demotion System | Review-only scorecard consumer now exists, feeds the guarded human review/apply evidence layer, and is visible through the local API/Obsidian review surface. Stage 4 can classify wallets from the integrated Stage 3 scorecard, `data/wallet_candidate_audit.json` can consume those classifications, and `/api/wallet-candidate-audit` exposes the queue without auto-applying changes. Automatic trust evolution remains future work. | 60% |
...
- <mark>Stage 4 - Wallet Promotion/Demotion System: 60%</mark>
...
- Stage 3 is complete as a wallet evidence engine. Stage 4 now consumes the scorecard conservatively, feeds the human decision/apply guard through `data/wallet_candidate_audit.json`, and exposes that review queue through `/api/wallet-candidate-audit` plus Obsidian wallet candidate review notes. Current local queue covers `49` candidates with `0` promotion reviews, `0` demotion reviews, `4` risk-review-required rows, `45` insufficient-evidence rows, `5` resolved rows, and `0` auto-applied changes. The next Stage 4 step is to add a narrow review-decision helper/report that prepares proposed approval records for selected wallets without applying them.
