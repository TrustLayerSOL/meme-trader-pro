...
- Archival mint history collection now exposes `/api/archival-mint-history-collection` and prepares the `34` mint accounts for complete-history collection. Current local dry-run has `34` pending mint-history targets, `0` complete histories, and `0` raw transactions preserved. It only marks a mint complete when signature pagination reaches the end of account history and all eligible decision-time transactions are preserved.
...
- [x] Archival mint history collector. `wallets/archival_mint_history_collector.py` and `utils/collect_archival_mint_history.py` generate `data/reports/historical_backfill/archival_mint_history_collection_report.json`, `data/wallet_backfills/raw_transactions/archival_mint_history_raw.jsonl`, and `data/reports/historical_backfill/archival_mint_history_completeness.json`. Current local dry-run prepares `34` mint-history targets, marks `0` complete, preserves `0` raw transactions, and exposes `/api/archival-mint-history-collection` as read-only. Execution is opt-in and only marks history complete when `getSignaturesForAddress` pagination reaches the end of the mint account history and every eligible transaction at or before the decision slot is fetched without budget truncation.
