...
- [x] Archival mint history collector. `wallets/archival_mint_history_collector.py` and `utils/collect_archival_mint_history.py` generate `data/reports/historical_backfill/archival_mint_history_collection_report.json`, `data/wallet_backfills/raw_transactions/archival_mint_history_raw.jsonl`, and `data/reports/historical_backfill/archival_mint_history_completeness.json`. Dry-run prepares `34` mint-history targets. `--max-targets` now caps execute batches. A bounded 3-target execute probe reached RPC and correctly blocked all `3` as `signature_page_limit_reached`, marking `0` complete and preserving `0` raw transactions. `/api/archival-mint-history-collection` exposes it as read-only. Execution is opt-in and only marks history complete when `getSignaturesForAddress` pagination reaches the end of the mint account history and every eligible transaction at or before the decision slot is fetched without budget truncation.
