...
- <mark>Proof-readiness blocker reduction after Stage 9. Current queue is score-ready market context, 15m outcome density, fillability, and archival supply evidence. Stage 10 - Semi-Autonomous Risk Engine remains 0% and deferred.</mark>
...
- Behavioral trust validation now confirms those Stage 9 patterns are not justified for wallet trust. Current local report validates `7` behavioral pattern candidates, marks `0` trust-ready, keeps `behavioral_trust_justified=false`, and blocks trust changes because proof readiness is `0%`, known 15m outcomes are `2` vs `30` required, fillable rate is `6%` vs `70%` required, and score-ready market-context records are `0`.
- Discord intelligence now sits inside Stage 9 as a sparse research-review layer. `research/discord_intelligence_layer.py`, `utils/build_discord_intelligence_layer.py`, `utils/dispatch_discord_intelligence.py`, and `/api/discord-intelligence-layer` prepare high-signal wallet-review, behavioral-pattern, replay-validation, regime-monitor, and evidence-milestone messages while keeping dispatch disabled by default. Dispatch now supports channel-specific local webhook routing through ignored config or env vars and still requires `--send`. The local report currently prepares `5` events, touches `5` channels, suppresses `18` raw wallet rows, keeps proof readiness at `0%`, and keeps live execution locked.
- Proof-readiness blocker reduction now ranks the exact next queue at `/api/proof-readiness-blocker-reduction`: `0` score-ready market-context records across `643` affected rows, `2` known 15m outcomes vs `30` required, `6%` fillable rate vs `70%` required, and `0` recovered archival supply rows across `643` rows. This is a queue, not trust permission.
- Score-ready market-context classification now narrows the first queue item at `/api/score-ready-market-context`: `643` rows scanned, `0` score-ready rows, `86` near-score-ready rows with price and liquidity present, `34` tokens needing archival supply, `23` wallets affected, `442` rows still missing price, and `115` rows still missing liquidity. This is classification only, not recovered proof.
...
- Archival mint snapshot request bundling now exposes `/api/archival-mint-snapshot-request-bundle`: `34` pending provider requests bundled, `34` target tokens, `0` provider calls, `0` wallet-list mutations, and `0` auto trust mutations. It gives the operator one saved-response handoff file and import command without storing provider URLs or secrets.
...
- Stage 10 is intentionally deferred because proof readiness is still `0%`. The next practical implementation step is not risk automation; it is raising proof readiness by improving known outcome labels and score-ready decision-time market context.
