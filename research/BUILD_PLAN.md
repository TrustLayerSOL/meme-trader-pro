...
| Stage 4 | Wallet Promotion/Demotion System | Review-only scorecard consumer now exists, feeds the guarded human review/apply evidence layer, is visible through the local API/Obsidian review surface, and can prepare draft-only decision records. Stage 4 can classify wallets from the integrated Stage 3 scorecard, `data/wallet_candidate_audit.json` can consume those classifications, `/api/wallet-candidate-audit` exposes the queue, and `/api/wallet-candidate-decision-prep` drafts non-approved proposed decisions without auto-applying changes. Automatic trust evolution remains future work. | 65% |
...
- <mark>Stage 4 - Wallet Promotion/Demotion System: 65%</mark>
...
- Stage 3 is complete as a wallet evidence engine. Stage 4 now consumes the scorecard conservatively, feeds the human decision/apply guard through `data/wallet_candidate_audit.json`, exposes that review queue through `/api/wallet-candidate-audit`, and prepares draft-only review decisions through `/api/wallet-candidate-decision-prep` plus `data/reports/wallet_reviews/wallet_candidate_decision_prep.json`. Current local queue covers `49` candidates with `0` promotion reviews, `0` demotion reviews, `4` risk-review-required rows, `45` insufficient-evidence rows, `5` resolved rows, `0` proposed draft approvals, and `0` auto-applied changes. The next Stage 4 step is to make the remaining non-actionable buckets easier to review before collecting more history.
