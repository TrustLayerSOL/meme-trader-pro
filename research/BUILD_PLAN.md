...
- <mark>Forward proof-data calibration after Stage 9. Historical/manual recovery remains available, but the main path is now preserving current wallet activity safely, capturing collection-time market context, resolving later outcomes, and making those outcomes visible beside Stage 8 proof readiness without treating them as historical proof. Current forward calibration shows `1,077` known 15m outcomes across `3,574` forward rows, all review-only with `0` trust/list mutations. The wallet-level forward scorecard finds `59` runner 15m outcomes, `1,018` flat 15m outcomes, `1` review-behavioral-signal wallet, `7` flat/noise candidate wallets, `23` wallets mostly blocked by missing entry context, and `1` collect-more-evidence wallet. The conservative recommendation layer allows `0` promotions from this forward batch. The forward collector, market-context capture, fixed-window resolver, daily calibration report, scorecard, recommendations, public-RPC preflight, adaptive degraded-provider throttling, free-RPC canary runner, and provider-rotation report are wired behind a no-paid-RPC default: current runs use public/free Solana RPC unless the operator explicitly passes `--allow-paid-rpc`. Public/free mode supports explicit fallback URLs; the provider rotation found `2` healthy public endpoints, and the two latest bounded free-RPC canaries both recommended `FREE_RPC_USABLE_SMALL_THROTTLED`. The latest canary processed `10` wallets, created `15` evidence rows, captured `15` market snapshots, and projected `8,640` RPC/day. Broad public-RPC schedules should still remain off until one more small canary stays stable. Stage 10 - Semi-Autonomous Risk Engine remains 0% and deferred.</mark>
