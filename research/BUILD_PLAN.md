...
- <mark>Forward proof-data calibration after Stage 9. Historical/manual recovery remains available, but the main path is now preserving current wallet activity safely, capturing collection-time market context, resolving later outcomes, and making those outcomes visible beside Stage 8 proof readiness without treating them as historical proof. Current forward calibration shows `1,009` known 15m outcomes across `3,547` forward rows, all review-only with `0` trust/list mutations. The new wallet-level forward calibration scorecard currently finds no runner/rug/dead/loser forward evidence, `8` flat/noise candidate wallets, and `22` wallets mostly blocked by missing entry context. The forward collector, market-context capture, fixed-window resolver, daily calibration report, and forward scorecard are wired behind a no-paid-RPC default: current runs use public/free Solana RPC unless the operator explicitly passes `--allow-paid-rpc`. Public/free mode supports explicit fallback URLs, but the safe operating shape is small and throttled because public endpoints rate-limit. Stage 10 - Semi-Autonomous Risk Engine remains 0% and deferred.</mark>
