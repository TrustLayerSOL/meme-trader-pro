...
- [x] Forward market snapshot capture. `wallets/forward_market_snapshot_capture.py` and `utils/capture_forward_market_snapshot_repair_queue.py` run a bounded review-only capture pass over the repair queue. Current local top-10 execution selected `10` queued mints, captured `3` usable current market snapshots, had `7` provider misses, and wrote isolated captured/combined context files. When the combined context file was routed through the isolated 250-row Helius quote-anchor merge, resolver-ready rows increased from `180` to `213`, blocked records decreased from `2,531` to `2,498`, known 15m outcomes stayed at `1,282`, and review-behavioral-signal wallets stayed at `4`. Canonical market context and resolver files were not overwritten; promotions, wallet-list mutations, trust mutations, and trades remained `0`.
