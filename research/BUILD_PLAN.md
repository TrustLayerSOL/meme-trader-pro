...
| Stage 4 | Wallet Promotion/Demotion System | Review-only scorecard consumer now exists, feeds the guarded human review/apply evidence layer, is visible through the local API/Obsidian review surface, can prepare draft-only decision records, and now summarizes review buckets. Stage 4 can classify wallets from the integrated Stage 3 scorecard, expose the queue, draft non-approved proposed decisions, and separate actionable, risk-review, insufficient-evidence, and resolved rows without auto-applying changes. Automatic trust evolution remains future work. | 70% |
...
- <mark>Stage 4 - Wallet Promotion/Demotion System: 70%</mark>
...
- Stage 3 is complete as a wallet evidence engine. Stage 4 now consumes the scorecard conservatively, feeds the human decision/apply guard through `data/wallet_candidate_audit.json`, exposes that review queue through `/api/wallet-candidate-audit`, prepares draft-only review decisions through `/api/wallet-candidate-decision-prep`, and summarizes the remaining review buckets through `/api/wallet-candidate-review-summary`. Current local queue covers `49` candidates with `0` actionable rows, `4` risk-review-required rows, `45` insufficient-evidence rows, `5` resolved rows, `0` proposed draft approvals, and `0` auto-applied changes. The next grounded step is to collect or backfill evidence for those non-actionable wallets rather than forcing trust changes from weak data.
