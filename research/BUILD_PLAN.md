...
- Archival mint history progress now exposes `/api/archival-mint-history-progress`: `75` requirements scanned, `75` checkpointed tokens, `48` tokens reached their decision slot, `37` reached decision slot but not account-history start, `27` remain before decision slot, and `11` have complete histories. This is progress visibility only, not supply proof.
- Archival mint pagination planning now exposes `/api/archival-mint-pagination-plan`: `75` tokens planned, `27` continue-pagination tokens, `37` provider-recommended / history-start-not-proven tokens, and `11` complete-history tokens ready for reconstruction. `utils/run_targeted_archival_mint_pagination.py` can consume that plan, filter low-cost continue-pagination targets, write a bounded filtered plan, and delegate collection to the existing mint-history collector without mutating trust or execution. The latest targeted run selected `8` low-cost targets, proved `0` new complete histories, and kept proof readiness at `11%`. This is the next-action planner for whether to keep paginating or evaluate an archival account-state provider.
...
- [x] Archival mint pagination planner. `wallets/archival_mint_pagination_planner.py` and `utils/build_archival_mint_pagination_plan.py` generate `data/reports/historical_backfill/archival_mint_pagination_plan_report.json` from mint-history progress. Current local report plans `75` tokens: `11` ready for supply reconstruction, `27` continue pagination toward the decision slot, and `37` consider archival account-state provider because the decision slot was reached without full account-history proof. `/api/archival-mint-pagination-plan` exposes it as read-only. `utils/run_targeted_archival_mint_pagination.py` adds an operator-safe runner for low-cost continue-pagination subsets; its latest bounded execute pass selected `8` targets and completed `0` additional mint histories.
