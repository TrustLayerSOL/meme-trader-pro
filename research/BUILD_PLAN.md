...
| Stage 4 | Wallet Promotion/Demotion System | Review-only scorecard consumer now exists, feeds the guarded human review/apply evidence layer, is visible through the local API/Obsidian review surface, can prepare draft-only decision records, summarizes review buckets, converts non-actionable rows into an evidence collection plan, and now has a read-only batch runner/API report to refresh the evidence chain. Stage 4 can classify wallets, expose the queue, draft non-approved proposed decisions, separate review buckets, identify exact next evidence steps, and rerun supporting evidence reports without auto-applying changes. Automatic trust evolution remains future work. | 80% |
...
- <mark>Stage 4 - Wallet Promotion/Demotion System: 80%</mark>
...
- Stage 3 is complete as a wallet evidence engine. Stage 4 now consumes the scorecard conservatively, feeds the human decision/apply guard through `data/wallet_candidate_audit.json`, exposes that review queue through `/api/wallet-candidate-audit`, prepares draft-only review decisions through `/api/wallet-candidate-decision-prep`, summarizes the remaining review buckets through `/api/wallet-candidate-review-summary`, converts them into `/api/wallet-candidate-collection-plan`, and refreshes the supporting report chain through `/api/wallet-candidate-collection-batch`. Current local batch covers `49` targets: `36` need outcomes plus market context, `9` need outcome labels, `4` need manual risk review, `5` resolved rows are excluded, `16/16` report-refresh steps passed, and `0` wallet-list changes are auto-applied. The next grounded step is a Stage 4 blocked-reason reducer that shows which missing evidence categories are still preventing decisions.
