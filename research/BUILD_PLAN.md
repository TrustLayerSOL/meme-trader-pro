...
| Stage 4 | Wallet Promotion/Demotion System | Review-only scorecard consumer now exists, feeds the guarded human review/apply evidence layer, is visible through the local API/Obsidian review surface, can prepare draft-only decision records, summarizes review buckets, converts non-actionable rows into an evidence collection plan, has a read-only batch runner/API report to refresh the evidence chain, reduces blocker reasons into one operator-readable report, builds a targeted context-recovery queue for the largest blocker bucket, and now runs a bounded local-artifact recovery linker over that queue. Stage 4 can classify wallets, expose the queue, draft non-approved proposed decisions, separate review buckets, identify exact next evidence steps, rerun supporting evidence reports, explain why wallets are still blocked, target the next evidence lane, and prove which blocked wallets have existing artifacts without auto-applying changes. Automatic trust evolution remains future work. | 95% |
...
- <mark>Stage 4 - Wallet Promotion/Demotion System: 95%</mark>
...
- Stage 3 is complete as a wallet evidence engine. Stage 4 now consumes the scorecard conservatively, feeds the human decision/apply guard through `data/wallet_candidate_audit.json`, exposes that review queue through `/api/wallet-candidate-audit`, prepares draft-only review decisions through `/api/wallet-candidate-decision-prep`, summarizes the remaining review buckets through `/api/wallet-candidate-review-summary`, converts them into `/api/wallet-candidate-collection-plan`, refreshes the supporting report chain through `/api/wallet-candidate-collection-batch`, explains the current blockers through `/api/wallet-candidate-blockers`, targets the largest blocker through `/api/wallet-candidate-context-recovery`, and links existing local artifacts for that lane through `/api/wallet-candidate-context-recovery-runner`. Current bounded recovery processed `36` wallets, linked `849` enriched evidence rows, `738` missing-market-context rows, and `623` historical backfill records. All `36` remain blocked from trust changes because recovered evidence is incomplete; `0` score-ready rows were created and `0` wallet-list changes were auto-applied. The next grounded step is a blocker-closeout report that separates outcome-label recovery, market-context recovery, transaction-linkage gaps, and manual risk review.
