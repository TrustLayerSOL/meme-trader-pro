...
- <mark>Forward proof-data collection after Stage 9. Historical/manual recovery remains available, but the main path is now collecting current wallet activity safely, capturing collection-time market context, resolving later outcomes, and producing daily calibration reports. The forward collector, market-context capture, fixed-window resolver, and daily calibration report are wired behind a no-paid-RPC default: current runs use public/free Solana RPC unless the operator explicitly passes `--allow-paid-rpc`. Public/free mode may collect less data because of external rate limits, but it prevents accidental Helius credit spend. Stage 10 - Semi-Autonomous Risk Engine remains 0% and deferred.</mark>
