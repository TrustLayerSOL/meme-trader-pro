...
- [x] Helius quote probe. `wallets/forward_helius_quote_probe.py` and `utils/run_forward_helius_quote_probe.py` generate bounded review-only exports under `data/reports/forward_testing/helius_quote_probe/`. Dry-run is the default; paid RPC requires explicit `--execute --allow-paid-rpc`. Current local probes fetched `6` / `6` Helius transaction bodies with `0` RPC failures and `0` recoverable same-transaction quote anchors under the existing parser, so Helius RPC is confirmed usable but the quote blocker likely needs enhanced transaction parsing, native SOL balance-delta handling, or separate market-context repair. The probe writes `0` repaired rows, allows `0` promotions, and performs `0` wallet-list/trust mutations.
