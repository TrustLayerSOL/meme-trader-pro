...
| Stage 4 | Wallet Promotion/Demotion System | Review-only scorecard consumer now exists and feeds the guarded human review/apply evidence layer. Stage 4 can classify wallets from the integrated Stage 3 scorecard, and `data/wallet_candidate_audit.json` can now consume those classifications as review-only promotion, demotion/block, risk-review, or insufficient-evidence rows with `0` auto-applied changes. Automatic trust evolution remains future work. | 50% |
...
- <mark>Stage 4 - Wallet Promotion/Demotion System: 50%</mark>
...
- Stage 3 is complete as a wallet evidence engine. Stage 4 now consumes the scorecard conservatively and feeds the human decision/apply guard through `data/wallet_candidate_audit.json`: current local queue covers `50` candidates with `0` promotion reviews, `1` demotion review, `4` risk-review-required rows, `45` insufficient-evidence rows, `4` resolved rows, and `0` auto-applied changes. The next Stage 4 step is to expose this Stage 4-backed audit queue through the local API/Obsidian review layer.
...
- [~] Wallet candidate audit. `wallets/wallet_candidate_audit.py` and `utils/build_wallet_candidate_audit.py` generate `data/wallet_candidate_audit.json` for snapshot-linked, replay-review, and Stage 4 promotion/demotion candidates. Current active queue has `0` promotion-review candidates, `1` demotion-review candidate, `4` risk-review-required rows, `45` insufficient-evidence rows, and `4` resolved rows. It is review-only and blocks wallet-list apply unless an explicit approved decision maps cleanly to current audit evidence.
