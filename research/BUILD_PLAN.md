...
- <mark>Forward proof-data collection after Stage 9. Historical/manual recovery remains available, but the main path is now collecting current wallet activity safely, capturing collection-time market context, resolving later outcomes, and producing daily calibration reports. The forward collector, market-context capture, fixed-window resolver, and daily calibration report are wired behind a developer-plan-safe budget: default `50` wallets per `900s` cycle, projected about `100,800` RPC calls/day, blocked above `120,000` projected RPC calls/day. The next missing piece is letting this lane run on fresh current data so enough replay-safe forward rows mature. Stage 10 - Semi-Autonomous Risk Engine remains 0% and deferred.</mark>
