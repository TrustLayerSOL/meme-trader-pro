...
- [x] Forward market snapshot capture. `wallets/forward_market_snapshot_capture.py` and `utils/capture_forward_market_snapshot_repair_queue.py` run bounded review-only capture passes over the repair queue. The tool now supports `--start-index` so later batches can skip already-tested queue rows while using the previous combined context file. Current local cumulative top-20 execution selected `20` queued mints across two batches, captured `5` usable current market snapshots, had `15` provider misses, and wrote isolated captured/combined context files. When the cumulative context file was routed through the isolated 250-row Helius quote-anchor merge, resolver-ready rows increased from `180` to `223`, blocked records decreased from `2,531` to `2,488`, known 15m outcomes stayed at `1,282`, and review-behavioral-signal wallets stayed at `4`. Canonical market context and resolver files were not overwritten; promotions, wallet-list mutations, trust mutations, and trades remained `0`.
