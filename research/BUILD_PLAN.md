...
- Focused provider-recommended capture/import now has dedicated wrappers for the `349` focused requests. Current dry-run capture sees `4` focused chunks and `349` requests, performs `0` provider calls, and blocks as expected without an executed provider call. Current focused import scans only those `349` provider-recommended requests, filters out `201` broad-batch rows, imports `0` snapshots because no raw provider responses exist yet, and keeps all trust/execution mutation locked.
