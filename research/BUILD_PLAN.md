...
- Discord intelligence now sits inside Stage 9 as a sparse research-review layer. `research/discord_intelligence_layer.py`, `utils/build_discord_intelligence_layer.py`, and `/api/discord-intelligence-layer` prepare high-signal wallet-review, behavioral-pattern, replay-validation, regime-monitor, and evidence-milestone messages while keeping dispatch disabled by default. The local report currently prepares `5` events, touches `5` channels, suppresses `18` raw wallet rows, keeps proof readiness at `0%`, and keeps live execution locked.
