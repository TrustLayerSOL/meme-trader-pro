...
- [~] Wallet candidate audit. `wallets/wallet_candidate_audit.py` and `utils/build_wallet_candidate_audit.py` generate `data/wallet_candidate_audit.json` for snapshot-linked and replay-review promotion/demotion candidates. Current active queue has `15` demotion-review candidates, `0` active promotion-review candidates, and `4` resolved applied replay promotions. It is review-only and blocks wallet-list apply unless an explicit approved decision maps cleanly to current audit evidence.
