...
- Archival supply recovery planning now turns the near-score-ready rows into `/api/archival-supply-recovery-plan`: `591` candidate rows, `591` rows with recovered decision slots, `75` token-level archival mint-supply requirements, `33` wallets affected, and `0` wallet trust/list mutations. This is an evidence shopping list only; it does not fetch supply, infer supply, substitute current supply, or change trust.
- Archival mint history collection now exposes `/api/archival-mint-history-collection` and prepares the `75` mint accounts for complete-history collection. Dry-run prepares `75` mint-history targets. The collector supports `--max-targets` for bounded execute batches and a resumable signature checkpoint so deeper runs do not refetch already-seen pages. Prior bounded execution checkpointed `3` sampled mints; current dry-run keeps all `75` targets pending until deeper collection or provider snapshots exist. It only marks a mint complete when signature pagination reaches the end of account history and all eligible decision-time transactions are preserved.
- Archival mint history progress now exposes `/api/archival-mint-history-progress`: `75` requirements scanned, `3` checkpointed tokens, `72` missing checkpoints, `1` token reached its decision slot but not account-history start, and `0` complete histories. This is progress visibility only, not supply proof.
- Archival mint pagination planning now exposes `/api/archival-mint-pagination-plan`: `75` tokens planned, `72` initial pagination tokens, `2` continue-pagination tokens, `1` provider-recommended token, and `0` complete histories. This is the next-action planner for whether to keep paginating or evaluate an archival account-state provider.
...
- [x] Archival mint supply reconstruction gate. `wallets/archival_mint_supply_reconstruction.py` and `utils/reconstruct_archival_mint_supply.py` generate `data/reports/historical_backfill/archival_mint_supply_reconstruction_report.json` and optional compatible snapshots from complete mint/burn history. Current local run scans the `75` token requirements, reconstructs `0` supply snapshots, and blocks all `75` as `incomplete_mint_history` because local wallet transaction artifacts do not prove complete mint history through the decision slot. `/api/archival-mint-supply-reconstruction` exposes it as read-only. This gate prevents partial local raw history from being mistaken for market-cap evidence.
- [x] Archival supply evidence import. `wallets/archival_supply_evidence.py` and `utils/build_archival_supply_evidence.py` generate `data/reports/historical_backfill/archival_supply_evidence_report.json` and `data/reports/historical_backfill/archival_supply_evidence_records.jsonl` from the archival supply recovery plan plus optional historical mint-account snapshots. Current local report scans `591` candidate rows, recovers `0` supply rows because no historical mint snapshots are present, blocks all `591` as `missing_archival_snapshot`, and affects `75` tokens / `33` wallets. `/api/archival-supply-evidence` exposes it as read-only. The score-ready classifier consumes recovered archival supply records when they exist and recomputes market cap from decision-time price times historical supply; current score-ready rows remain limited to prior local market-cap snapshots until real snapshots are imported.
