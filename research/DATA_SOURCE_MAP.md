...
| `data/wallet_review_decisions.json` | Operator approvals for wallet promotions/demotions. Only approved decisions are eligible for the apply tool. Replay-based review recommendations can be synced here, but wallet-list apply still requires the existing apply gates. | wallet apply utility, native Wallets review actions | operator/manual edit, `POST /api/wallet-review-decision`, `utils/sync_replay_wallet_decisions.py` |
...
| `data/wallet_candidate_audit.json` | Review-only audit report for snapshot-linked promotion/demotion candidates. Summarizes sample gates, known outcomes, runner/rug/dead counts, source coverage, recommendation evidence, and baseline comparison status. Explicitly blocks wallet-list apply. | operator/research review | `utils/build_wallet_candidate_audit.py`, `wallets.wallet_candidate_audit` |
...
| Wallet candidate audit | `data/wallet_candidate_audit.json` | `data/wallet_outcome_ledger.json`, `data/wallet_baseline_comparison.json` | Human-review gate for generated promotion/demotion candidates. It is not consumed by wallet-list apply. |
...
| Wallet replay scorecard | `data/wallet_replay_scorecard.json` | `data/historical_replay/replay_events.jsonl` | Review-only behavioral scorecard. Separates known coverage from outcome performance and exposes repeated co-entry behavior. It is not promotion logic yet. |
...
| Wallet candidate audit | per-candidate review packet with apply blocked, evidence gates, known outcome sample, source coverage, runner/rug/dead counts, average PnL, confidence, and recommendation reasons | `wallets/wallet_candidate_audit.py`, `utils/build_wallet_candidate_audit.py`, `data/wallet_candidate_audit.json` |
