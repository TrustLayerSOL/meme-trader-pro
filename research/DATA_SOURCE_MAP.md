...
| `/api/positions/{mint}` | positions plus SQLite `token_snapshots`, `data/social_state.json`, `data/catalyst_cards.json`, `data/wallet_performance.json`, `data/wallet_behavior.json`, SQLite-first `/api/trades` paper outcomes | Selected-position detail payload with snapshot trend metrics, read-only protection summary, local signal/catalyst matches, and wallet confidence context. Wallet context now uses the canonical SQLite-first trade read path when explicit paper state is not supplied. |
...
| `/api/trades` | SQLite `trades`, fallback `data/paper_trades.json` | Open/closed/failed trade state. Canonical read path now prefers durable SQLite trades and falls back to JSON paper trade state when SQLite has no trade rows. Feeds the desktop Portfolio view, static desktop Portfolio panel, paper replay, and selected-token trade lifecycle. |
...
- SQLite is useful for querying and persistence, but several panels still read JSON directly. Treat SQLite as durable memory, not yet the only source of truth. `/api/alerts` now prefers SQLite alert rows with a live-state fallback, and `/api/trades` now prefers SQLite trade rows with a paper-trades JSON fallback.
