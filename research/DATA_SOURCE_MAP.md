...
| `data/historical_replay/replay_events.jsonl` | Generated review-only historical replay event dataset. Normalizes accepted trades, failed trades, rejected signals, and wallet observations into one replay event schema while keeping later outcome labels outside decision context. Each event declares fixed `30s`, `2m`, `5m`, and `15m` evaluation windows plus latency/slippage/liquidity/failed-fill assumptions. Ignored by git as local generated research data. | historical replay analysis, leakage checks, wallet feedback-loop validation | `utils/build_historical_replay_dataset.py`, `research.historical_replay_dataset`, `utils.build_wallet_outcome_ledger` |
| `data/historical_replay/summary.json` | Compact summary for the generated historical replay dataset: event counts, unsafe/leakage count, fill-status counts, evaluation windows, mode, schema, and output path. Ignored by git as local generated research data. | operator/research review | `utils/build_historical_replay_dataset.py` |
...
| Historical replay dataset | accepted/rejected/failed records converted into decision-time-safe replay events with fixed windows, execution assumptions, fill-status classification, and separated later outcome labels | `research/historical_replay_dataset.py`, `utils/build_historical_replay_dataset.py`, `data/historical_replay/replay_events.jsonl` |
