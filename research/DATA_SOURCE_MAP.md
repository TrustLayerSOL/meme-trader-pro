...
| `/api/wallet-replay-review` | `data/wallet_replay_scorecard.json` | Read-only replay scorecard triage endpoint. Separates reviewable wallets from low-coverage wallets using known/fillable replay evidence, surfaces repeated co-entry pairs for ecosystem review, and includes a markdown operator report for human review/export. The Obsidian exporter also renders this as `MemeTraderPro/Dashboards/Wallet Replay Ecosystem Review.md`. |
