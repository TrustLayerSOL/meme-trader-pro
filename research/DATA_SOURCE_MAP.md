...
| `data/wallet_outcome_ledger.json` | Review-only wallet outcome ledger generated from unified accepted-trade and rejected-signal records. Aggregates total signals, accepted/rejected counts, known outcomes, runner/rug/dead participation, average PnL after signal, liquidity, token age, cluster duration, market-regime breakdown, promotion/demotion score, confidence, and recommendation. Later outcomes are labeled by `research.outcome_labeler`; review recommendations are owned by `wallets.wallet_promotion_engine`. | future wallet promotion/demotion review | `utils/build_wallet_outcome_ledger.py`, `wallets.wallet_outcome_ledger`, `wallets.wallet_promotion_engine`, `research.signal_schema`, `research.outcome_labeler` |
...
| Wallet outcome ledger | `data/wallet_outcome_ledger.json` | `data/paper_trades.json`, `data/rejected_signals/rejections.jsonl`, `research.signal_schema` | First durable review artifact exists. It is generated, not live trading logic. |
