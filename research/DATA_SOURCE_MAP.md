...
| `data/wallet_quant_report.json` | Target Wallet Quant Tracker report with tier, metric, evidence, recommendation, and reason rows. This is the next primary wallet-evaluation artifact. | future wallet quant endpoint/UI, operator review | planned `utils/build_wallet_quant_report.py` |
...
| Wallet quant report | planned `data/wallet_quant_report.json` | `data/wallet_performance.json`, `data/wallet_behavior.json`, `data/candidate_wallets.json`, `data/paper_watch_wallets.json`, SQLite `events/trades/token_snapshots` | New active priority. This report should become the operator's main wallet-evaluation surface before more GUI or strategy work. |
