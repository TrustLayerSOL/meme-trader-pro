...
| Unified signal outcome record | accepted/rejected/failed record type, wallets, signal context, trade/skip decision, later token outcome, replay assumptions, research safety flags | `research/signal_schema.py`, future wallet-outcome ledger |
| Wallet outcome ledger | per-wallet total/accepted/rejected signals, known outcomes, runner/rug/dead participation, average PnL, average liquidity, average token age, cluster duration, market-regime breakdown, confidence, promotion/demotion scores | `wallets/wallet_outcome_ledger.py`, `utils/build_wallet_outcome_ledger.py`, `data/wallet_outcome_ledger.json` |
