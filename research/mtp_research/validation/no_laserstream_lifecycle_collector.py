...
def format_no_laserstream_status(status: dict[str, Any]) -> str:
    ...
    lines = [
            ...
            f"Confirmed reached 50k / 100k / 500k / 1M: {status.get('confirmed_reached_50k', 0)} / {status.get('confirmed_reached_100k', 0)} / {status.get('confirmed_reached_500k', 0)} / {status.get('confirmed_reached_1m', 0)}",
            f"Unconfirmed reached 50k / 100k / 500k / 1M: {status.get('unconfirmed_reached_50k', 0)} / {status.get('unconfirmed_reached_100k', 0)} / {status.get('unconfirmed_reached_500k', 0)} / {status.get('unconfirmed_reached_1m', 0)}",
