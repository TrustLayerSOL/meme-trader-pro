...
def _birth_freshness_status(root: Path) -> dict[str, Any]:
    ...
    if not path.exists():
        return {
            ...
            "first_followup_before_any_trade_if_known": 0,
            ...
            "median_seconds_create_observed_to_first_followup_attempt": None,
            "median_seconds_create_to_first_path": None,
            "attempt_within_5s_pct": None,
            "observed_to_attempt_within_5s_pct": None,
    ...
    return {
        ...
        "first_followup_before_any_trade_if_known": payload.get("first_followup_before_any_trade_if_known", 0),
        ...
        "median_seconds_create_observed_to_first_followup_attempt": payload.get(
        ...
        "observed_to_attempt_within_5s_pct": payload.get("observed_to_attempt_within_5s_pct"),
...
def status_markdown(payload: dict[str, Any]) -> str:
    lines = [
        ...
        f"- First follow-up before any trade if known: `{(payload.get('birth_watch_freshness') or {}).get('first_followup_before_any_trade_if_known', 0)}`",
        ...
        f"- Median create-observed-to-first-attempt seconds: `{(payload.get('birth_watch_freshness') or {}).get('median_seconds_create_observed_to_first_followup_attempt')}`",
        f"- Create-to-first-attempt within 5s pct: `{(payload.get('birth_watch_freshness') or {}).get('attempt_within_5s_pct')}`",
        f"- Create-observed-to-first-attempt within 5s pct: `{(payload.get('birth_watch_freshness') or {}).get('observed_to_attempt_within_5s_pct')}`",
    ...
    return "\n".join(
        [
            ...
            *_format_source_readiness(tally.get("source_readiness") or []),
            ...
            f"First follow-up before any trade if known: {(tally.get('birth_watch_freshness') or {}).get('first_followup_before_any_trade_if_known', 0)}",
            ...
            f"Median create-observed-to-first-attempt seconds: {(tally.get('birth_watch_freshness') or {}).get('median_seconds_create_observed_to_first_followup_attempt')}",
            f"Create-to-first-attempt within 5s pct: {(tally.get('birth_watch_freshness') or {}).get('attempt_within_5s_pct')}",
            f"Create-observed-to-first-attempt within 5s pct: {(tally.get('birth_watch_freshness') or {}).get('observed_to_attempt_within_5s_pct')}",
