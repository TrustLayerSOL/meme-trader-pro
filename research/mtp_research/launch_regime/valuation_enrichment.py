...
def run_valuation_enrichment(
    *,
    ...
    snapshots = [enrich_snapshot_row(row) for row in _load_jsonl(Path(snapshots_path))]
    outcomes = [enrich_outcome_row(row) for row in _load_jsonl(Path(outcomes_path))]
...
def _apply_valuation_fields(
    ...
    *,
    ...
    row.update(
        {
            ...
            "valuation_confidence": "liquidity_proxy_only" if liquidity_proxy_sol is not None else "missing",
    ...
    for threshold in VALUATION_THRESHOLDS:
        row[f"ever_hit_valuation_proxy_{threshold // 1000}k"] = None
