...
| Rug association | Wallet participation frequency in rug-like or failed tokens. | `data/wallet_behavior.json`, future wallet-outcome ledger | After token outcome classification. | Penalizes wallets repeatedly appearing in dangerous launches. | Yes, only when prior classified history is used. | experimental |
...
| Entry timing quality | How early/late wallet entries occur relative to launch and market movement. | wallet behavior report, future wallet-outcome ledger | After enough signal history; prior history only for decisions. | Measures whether wallets enter before opportunity or after the move. | Yes, only when prior history is timestamp-filtered. | experimental |
...
| Later token outcome | Outcome label after the signal: runner, rug, dead, PnL path, drawdown, etc. | paper trades, postmortems, future replay/outcome ledger | Future label only. | Used for evaluation and scoring calibration. | No for decision input; yes as separated evaluation label. | watchlist |
