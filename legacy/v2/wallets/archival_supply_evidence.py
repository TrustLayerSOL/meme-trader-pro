...
def classify_candidate(candidate: dict[str, Any], snapshots_by_token: dict[str, list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    return {
        ...
        "evidence_key": snapshot.get("evidence_key") if isinstance(snapshot, dict) else None,
        "request_id": snapshot.get("request_id") if isinstance(snapshot, dict) else None,
        "evidence_url": snapshot.get("evidence_url") if isinstance(snapshot, dict) else None,
        "confidence_tier": snapshot.get("confidence_tier") if isinstance(snapshot, dict) else None,
        "notes": snapshot.get("notes") if isinstance(snapshot, dict) else None,
        "max_acceptable_snapshot_slot": snapshot.get("max_acceptable_snapshot_slot") if isinstance(snapshot, dict) else None,
