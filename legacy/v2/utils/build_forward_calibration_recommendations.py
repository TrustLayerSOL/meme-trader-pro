...
from wallets.forward_calibration_recommendations import build_forward_calibration_recommendations  # noqa: E402


DEFAULT_SCORECARD = ROOT / "data" / "reports" / "forward_testing" / "forward_calibration_scorecard.json"
DEFAULT_REPORT = ROOT / "data" / "reports" / "forward_testing" / "forward_calibration_recommendations.json"
DEFAULT_MARKDOWN = ROOT / "data" / "reports" / "forward_testing" / "forward_calibration_recommendations.md"
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# MemeTraderPro Forward Calibration Recommendations",
        "",
        "Conservative review-only recommendations from the forward calibration scorecard.",
        "",
        "## Summary",
        "",
    ...
    lines.extend(
        [
            ...
            "- Flat-only evidence does not promote a wallet.",
...
def write_forward_calibration_recommendations(
    *,
    ...
    report = build_forward_calibration_recommendations(read_json(scorecard_path), generated_at=generated_at)
    report["input_paths"] = {"forward_calibration_scorecard": relative_path(scorecard_path)}
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build conservative review-only forward calibration recommendations.")
