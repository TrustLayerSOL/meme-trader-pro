...
from wallets.supply_stability_evidence import build_supply_stability_evidence_report  # noqa: E402
...
DEFAULT_POST_DECISION_COMPLETENESS_UPDATES_PATH = (
    ROOT / "data" / "reports" / "historical_backfill" / "post_decision_supply_completeness_updates.json"
)
DEFAULT_REPORT_PATH = ROOT / "data" / "reports" / "historical_backfill" / "supply_stability_evidence_report.json"
DEFAULT_SNAPSHOTS_PATH = ROOT / "data" / "reports" / "historical_backfill" / "supply_stability_evidence_snapshots.jsonl"
...
def merge_history_completeness(
    base_completeness: dict[str, Any],
    post_decision_updates: dict[str, Any],
) -> dict[str, Any]:
    merged: dict[str, Any] = {}
    if isinstance(base_completeness, dict):
        for token, row in base_completeness.items():
            merged[str(token)] = dict(row) if isinstance(row, dict) else {}
    if isinstance(post_decision_updates, dict):
        for token, row in post_decision_updates.items():
            if not isinstance(row, dict):
                continue
            token_key = str(token)
            current = dict(merged.get(token_key, {}))
            update = dict(row)
            if "source" in update:
                update["post_decision_source"] = update.pop("source")
            current.update(update)
            merged[token_key] = current
    return merged


def write_supply_stability_evidence_report(
    *,
    ...
    post_decision_completeness_updates_path: Path | str = DEFAULT_POST_DECISION_COMPLETENESS_UPDATES_PATH,
    ...
    post_decision_completeness_updates_path = Path(post_decision_completeness_updates_path)
    ...
    history_completeness = merge_history_completeness(
        read_json(completeness_path, {}),
        read_json(post_decision_completeness_updates_path, {}),
    )
    ...
    report["input_paths"] = {
        ...
        "post_decision_completeness_updates": relative_path(post_decision_completeness_updates_path, ROOT),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build review-only current-supply stability evidence.")
    ...
    parser.add_argument("--post-decision-completeness-updates-path", type=Path, default=DEFAULT_POST_DECISION_COMPLETENESS_UPDATES_PATH)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = write_supply_stability_evidence_report(
        ...
        post_decision_completeness_updates_path=args.post_decision_completeness_updates_path,
