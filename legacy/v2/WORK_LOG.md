...
- `wallets/wallet_candidate_evidence_plan.py`
- `utils/build_wallet_candidate_evidence_plan.py`
- `desktop_api.py`
- `obsidian_export/exporter.py`
- `obsidian_export/intelligence_notes.py`
- `tests/test_wallet_candidate_evidence_plan.py`
- `tests/test_desktop_api.py`
- `tests/test_obsidian_export.py`
- `WORK_LOG.md`
- `research/BUILD_PLAN.md`
...
- Added a review-only evidence collection plan for the strongest candidate wallets.
- The plan converts the candidate-quality review shortlist into concrete missing evidence counts: replay-known 15m outcomes, fillable replay events, and outcome-ledger labels.
- Added `data/wallet_candidate_evidence_plan.json`.
- Current local plan counts: `50` candidates, `0` ready for review, `44` needing replay coverage, `44` needing outcome coverage, and `6` needing risk review.
- Added read-only `/api/wallet-candidate-evidence-plan` and Obsidian `MemeTraderPro/Dashboards/Wallet Candidate Evidence Plan.md`.

Verification:

- Added failing tests first for missing replay/outcome coverage, risk-review separation, report writing, read-only API routing, and Obsidian note rendering.
- `./trading_env/bin/python -m unittest tests.test_wallet_candidate_evidence_plan`
- `./trading_env/bin/python -m unittest tests.test_desktop_api.DesktopApiTests.test_wallet_candidate_evidence_plan_route_is_read_only`
- `./trading_env/bin/python -m unittest tests.test_obsidian_export.ObsidianExportTests.test_intelligence_notes_surface_anomalies_drift_lineage_and_workflow`
- `./trading_env/bin/python utils/build_wallet_candidate_evidence_plan.py`

Remaining risk / next step:

- This is a queue, not a collector. It identifies which evidence is missing but does not fetch new chain history.
- Next step is to use this plan to target replay/outcome backfill for the highest-quality wallets instead of expanding wallet count blindly.
...
- Current local report counts: `1,913` active candidates, `15` blocked candidates, `7` strong observations, `229` paper-watch review rows, `84` hold-review rows, and `1,593` reject-review rows.
...
Ignored local runtime restored/refreshed:

- `trading_env/`
- `data/memetrader.db`
- `data/process_locks/desktop_api.nohup.pid`
- `logs/desktop_api.log`

What changed:

- Confirmed the active working repo is `/Users/dianeposs/projects/meme-trader-pro`.
- Restored the repo to branch `phase6-protection-exits` at `148a851 Add missing market context targets`.
- Verified tracked source integrity against `origin/phase6-protection-exits`: `274/274` tracked files match, with `0` missing and `0` different.
- Confirmed the previously missing `apps/desktop` tracked subtree is restored from GitHub.
- Preserved the pre-repair copied overlay in git stash as `pre-branch-repair recovered overlay 2026-05-15` for safety.
- Recreated the local Python virtual environment from `requirements.txt`.
- Rebuilt `data/memetrader.db` through `utils/sync_state_to_sqlite.py --rebuild-trades`.
- Started the desktop API from the new non-iCloud path on `127.0.0.1:8765`.

Verification:

- Focused recovery tests passed: `112` tests.
- Python compile check passed for desktop API, storage, wallet backfill/enrichment, missing-market-context, Obsidian export, and SQLite sync modules.
- SQLite rebuild parity passed: expected paper trades matched actual SQLite trades.
- SQLite now contains `300` events, `200` alerts, `42` trades, `1` watchlist row, `0` token snapshots, `0` swap ticks, and `0` decision records.
- `/api/health` returned `200`, `status=ok`, `mode=EXECUTION_LOCKED`, and `live_execution_locked=true`.
- `/api/trades` returned `200` from `sqlite_trades` with `2` open, `33` closed, and `7` failed trades.
- Wallet review/recovery endpoints returned `200` and stayed read-only with live execution locked.

Remaining:

- The migrated SQLite market-history tables cannot be recovered from the missing old DB; `token_snapshots` and `swap_ticks` are empty until regenerated by future runtime/backfill work.
- `/api/readiness` still reports `FAIL`, which is expected while runtime/live readiness gates are not being targeted and live execution remains locked.
- Next step is to continue Quant Wallet Tracker V2 from the clean migrated repo: enrich/replay wallet evidence from the surviving JSON reports and regenerate missing market context where available.
...
### 2026-05-18 - Targeted Pagination Buckets
...
- Added `--min-estimated-pages` support to the targeted archival mint pagination runner so it can run page-cost buckets without repeatedly selecting the same lowest-cost targets.
- Added a regression test proving the runner can select a bounded min/max estimated-page bucket deterministically.
- Ran the `11-25` estimated-page bucket: dry-run selected `7` targets, execute processed `7`, completed `0` histories, and preserved `0` new raw rows.
- Ran the `26-40` estimated-page bucket: dry-run selected `3` targets, execute processed `3`, completed `0` histories, and preserved `936` raw mint-account transaction rows.
- Rebuilt the archival progress, pagination, reconstruction, supply evidence, score-ready market-context, replay timeline, Stage 6, Stage 8, validation/proof, behavioral validation, and blocker-reduction reports.
...
- Mint requirements scanned: `75`
- Checkpointed tokens: `75`
- Tokens that reached decision slot: `49`
- Complete mint histories: `11`
- Continue-pagination tokens: `26`
...
- `./trading_env/bin/python -m unittest tests.test_targeted_archival_mint_pagination tests.test_archival_mint_history_collector tests.test_archival_mint_pagination_planner tests.test_archival_mint_history_progress`
- `./trading_env/bin/python -m unittest`
- `./trading_env/bin/python -m py_compile utils/run_targeted_archival_mint_pagination.py utils/collect_archival_mint_history.py wallets/archival_mint_pagination_planner.py`
- `git diff --check`
...
### 2026-05-21 - Forward Entry Context Resolver
...
- Stage 8 validation contract remains `100%`.
- Forward entry-context resolver is complete as a review-only repaired-record artifact. It does not raise proof readiness, promote wallets, mutate wallet lists, or change execution.

Changed files:

- `wallets/forward_entry_context_resolver.py`
- `utils/build_forward_entry_context_resolver.py`
- `tests/test_forward_entry_context_resolver.py`
- `research/BUILD_PLAN.md`
- `research/DATA_SOURCE_MAP.md`
- `WORK_LOG.md`
...
- Added a guarded resolver for rows blocked by `missing_forward_entry_price`.
- Resolver only repairs rows with a valid same-transaction execution-price quote and at least one later market snapshot at or after the signal time.
- It writes repaired records separately from the original forward outcome records and writes rejected rows separately for audit.
- Current local resolver scanned `2,538` blocked rows, found `562` quote-anchor candidates, resolved `82` rows, produced `59` known 15m outcomes, blocked `480` quote-anchor rows with no later market snapshot, and blocked `1,976` rows with no valid quote anchor.
- Wallet-list mutations and trust mutations remain `0`.

Verification:

- `python3 -m pytest tests/test_forward_entry_context_resolver.py -q`
...
### 2026-05-21 - Small Free-RPC Forward Canary
...
- Stage 8 validation contract remains `100%`.
- Free/current forward evidence collection improved from unstable smoke output to a usable small-throttled canary result.
...
- Ran a bounded no-paid-RPC canary using the healthy public endpoint list.
- The canary processed `10` wallets, created `16` evidence rows, captured `14` market snapshots, projected `8,640` RPC/day, and recommended `FREE_RPC_USABLE_SMALL_THROTTLED` with no blockers.
- Rebuilt forward outcome visibility:
  - `3,569` forward rows scanned,
  - `1,074` merged known 15m outcomes,
  - `59` runner 15m outcomes,
  - `1,015` flat 15m outcomes,
  - `8` pending windows,
...
- Run one more small canary later with the same public endpoints. If two or three consecutive canaries stay usable and under budget, consider a conservative recurring schedule; otherwise keep collection manual/on-demand.

### 2026-05-21 - Second Small Free-RPC Forward Canary
...
- Stage 8 validation contract remains `100%`.
- Free/current forward evidence collection now has `2` consecutive small canaries recommending `FREE_RPC_USABLE_SMALL_THROTTLED`.
...
- Ran a second bounded no-paid-RPC canary using the same public endpoint list.
- The canary processed `10` wallets, created `15` evidence rows, captured `15` market snapshots, projected `8,640` RPC/day, and recommended `FREE_RPC_USABLE_SMALL_THROTTLED` with no blockers.
- Rebuilt forward outcome visibility:
  - `3,574` forward rows scanned,
  - `1,077` merged known 15m outcomes,
  - `59` runner 15m outcomes,
  - `1,018` flat 15m outcomes,
  - `9` pending windows,
  - `23` wallets mostly blocked by missing context,
  - `1` collect-more-evidence wallet.
- Promotions remain `0`, wallet-trust mutations remain `0`, wallet-list mutations remain `0`, and live execution remains locked.
