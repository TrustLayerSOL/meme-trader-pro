...
class DesktopApiTests(unittest.TestCase):
    ...
    def test_discord_intelligence_layer_payload_is_review_only(self):
        payload = desktop_api.build_discord_intelligence_layer_payload({
            "discord_intelligence_layer": {
                "mode": "DISCORD_BEHAVIORAL_INTELLIGENCE_REVIEW_ONLY",
                "summary": {"discord_intelligence_layer_completion_pct": 100, "events_prepared": 2},
                "discord_events": [{"event_id": "event-1"}, {"event_id": "event-2"}],
            }
        })

        self.assertEqual(payload["mode"], "DISCORD_BEHAVIORAL_INTELLIGENCE_REVIEW_ONLY")
        self.assertTrue(payload["read_only"])
        self.assertTrue(payload["review_only"])
        self.assertTrue(payload["live_execution_locked"])
        self.assertFalse(payload["wallet_list_apply_allowed"])
        self.assertFalse(payload["wallet_list_mutated"])
        self.assertFalse(payload["auto_trust_mutation_allowed"])
        self.assertFalse(payload["discord_dispatch_enabled"])
        self.assertEqual(payload["count"], 2)

    def test_discord_intelligence_layer_route_is_read_only(self):
        with mock.patch.object(desktop_api, "build_discord_intelligence_layer_payload", return_value={"mode": "DISCORD_BEHAVIORAL_INTELLIGENCE_REVIEW_ONLY", "live_execution_locked": True}) as build_payload:
            status, content_type, body = desktop_api.route_request("GET", "/api/discord-intelligence-layer")
    ...
    def test_archival_supply_recovery_plan_payload_is_review_only(self):
        payload = desktop_api.build_archival_supply_recovery_plan_payload({
            "archival_supply_recovery_plan": {
                "mode": "ARCHIVAL_SUPPLY_RECOVERY_PLAN_REVIEW_ONLY",
                "summary": {"candidate_rows": 86, "tokens_to_fetch": 34},
                "token_requirements": [{"token_mint": "MintA"}],
            }
        })

        self.assertEqual(payload["mode"], "ARCHIVAL_SUPPLY_RECOVERY_PLAN_REVIEW_ONLY")
        self.assertTrue(payload["read_only"])
        self.assertTrue(payload["review_only"])
        self.assertTrue(payload["live_execution_locked"])
        self.assertFalse(payload["wallet_list_apply_allowed"])
    ...
    def test_archival_supply_recovery_plan_route_is_read_only(self):
        with mock.patch.object(desktop_api, "build_archival_supply_recovery_plan_payload", return_value={"mode": "ARCHIVAL_SUPPLY_RECOVERY_PLAN_REVIEW_ONLY", "live_execution_locked": True}) as build_payload:
            status, content_type, body = desktop_api.route_request("GET", "/api/archival-supply-recovery-plan")
    ...
    def test_archival_supply_evidence_payload_is_review_only(self):
        payload = desktop_api.build_archival_supply_evidence_payload({
            "archival_supply_evidence": {
                "mode": "ARCHIVAL_SUPPLY_EVIDENCE_REVIEW_ONLY",
                "summary": {"candidate_rows": 86, "supply_recovered_records": 0},
                "records": [{"token_mint": "MintA", "status": "blocked_missing_archival_snapshot"}],
            }
        })

        self.assertEqual(payload["mode"], "ARCHIVAL_SUPPLY_EVIDENCE_REVIEW_ONLY")
        self.assertTrue(payload["read_only"])
        self.assertTrue(payload["review_only"])
        self.assertTrue(payload["live_execution_locked"])
        self.assertFalse(payload["wallet_list_apply_allowed"])
    ...
    def test_archival_supply_evidence_route_is_read_only(self):
        with mock.patch.object(desktop_api, "build_archival_supply_evidence_payload", return_value={"mode": "ARCHIVAL_SUPPLY_EVIDENCE_REVIEW_ONLY", "live_execution_locked": True}) as build_payload:
            status, content_type, body = desktop_api.route_request("GET", "/api/archival-supply-evidence")
    ...
    def test_archival_mint_snapshot_collection_payload_is_review_only(self):
        payload = desktop_api.build_archival_mint_snapshot_collection_payload({
            "archival_mint_snapshot_collection": {
                "mode": "ARCHIVAL_MINT_SUPPLY_SNAPSHOT_COLLECTION_REVIEW_ONLY",
                "summary": {"requests_prepared": 34, "snapshots_collected": 0},
                "requests": [{"token_mint": "MintA", "status": "pending_archival_provider"}],
            }
        })

        self.assertEqual(payload["mode"], "ARCHIVAL_MINT_SUPPLY_SNAPSHOT_COLLECTION_REVIEW_ONLY")
        self.assertTrue(payload["read_only"])
        self.assertTrue(payload["review_only"])
        self.assertTrue(payload["live_execution_locked"])
        self.assertFalse(payload["wallet_list_apply_allowed"])
    ...
    def test_archival_mint_snapshot_collection_route_is_read_only(self):
        with mock.patch.object(desktop_api, "build_archival_mint_snapshot_collection_payload", return_value={"mode": "ARCHIVAL_MINT_SUPPLY_SNAPSHOT_COLLECTION_REVIEW_ONLY", "live_execution_locked": True}) as build_payload:
            status, content_type, body = desktop_api.route_request("GET", "/api/archival-mint-snapshot-collection")
    ...
    def test_archival_mint_history_collection_payload_is_review_only(self):
        payload = desktop_api.build_archival_mint_history_collection_payload({
            "archival_mint_history_collection": {
                "mode": "ARCHIVAL_MINT_HISTORY_COLLECTION_REVIEW_ONLY",
                "summary": {"mint_history_targets": 34, "mint_histories_complete": 0},
                "targets": [{"token_mint": "MintA", "status": "pending_mint_history_collection"}],
            }
        })

        self.assertEqual(payload["mode"], "ARCHIVAL_MINT_HISTORY_COLLECTION_REVIEW_ONLY")
        self.assertTrue(payload["read_only"])
        self.assertTrue(payload["review_only"])
        self.assertTrue(payload["live_execution_locked"])
        self.assertFalse(payload["wallet_list_apply_allowed"])
    ...
    def test_archival_mint_history_collection_route_is_read_only(self):
        with mock.patch.object(desktop_api, "build_archival_mint_history_collection_payload", return_value={"mode": "ARCHIVAL_MINT_HISTORY_COLLECTION_REVIEW_ONLY", "live_execution_locked": True}) as build_payload:
            status, content_type, body = desktop_api.route_request("GET", "/api/archival-mint-history-collection")
    ...
    def test_archival_mint_history_progress_payload_is_review_only(self):
        payload = desktop_api.build_archival_mint_history_progress_payload({
            "archival_mint_history_progress": {
                "mode": "ARCHIVAL_MINT_HISTORY_PROGRESS_REVIEW_ONLY",
                "summary": {"checkpointed_tokens": 3},
                "rows": [{"token_mint": "MintA", "status": "checkpoint_before_decision_not_reached"}],
            }
        })

        self.assertEqual(payload["mode"], "ARCHIVAL_MINT_HISTORY_PROGRESS_REVIEW_ONLY")
        self.assertTrue(payload["read_only"])
        self.assertTrue(payload["review_only"])
        self.assertTrue(payload["live_execution_locked"])
        self.assertFalse(payload["wallet_list_apply_allowed"])
    ...
    def test_archival_mint_history_progress_route_is_read_only(self):
        with mock.patch.object(desktop_api, "build_archival_mint_history_progress_payload", return_value={"mode": "ARCHIVAL_MINT_HISTORY_PROGRESS_REVIEW_ONLY", "live_execution_locked": True}) as build_payload:
            status, content_type, body = desktop_api.route_request("GET", "/api/archival-mint-history-progress")
    ...
    def test_archival_mint_pagination_plan_payload_is_review_only(self):
        payload = desktop_api.build_archival_mint_pagination_plan_payload({
            "archival_mint_pagination_plan": {
                "mode": "ARCHIVAL_MINT_PAGINATION_PLAN_REVIEW_ONLY",
                "summary": {"provider_recommended_tokens": 1},
                "rows": [{"token_mint": "MintA", "recommended_action": "CONSIDER_ARCHIVAL_ACCOUNT_STATE_PROVIDER"}],
            }
        })

        self.assertEqual(payload["mode"], "ARCHIVAL_MINT_PAGINATION_PLAN_REVIEW_ONLY")
        self.assertTrue(payload["read_only"])
        self.assertTrue(payload["review_only"])
        self.assertTrue(payload["live_execution_locked"])
        self.assertFalse(payload["wallet_list_apply_allowed"])
    ...
    def test_archival_mint_pagination_plan_route_is_read_only(self):
        with mock.patch.object(desktop_api, "build_archival_mint_pagination_plan_payload", return_value={"mode": "ARCHIVAL_MINT_PAGINATION_PLAN_REVIEW_ONLY", "live_execution_locked": True}) as build_payload:
            status, content_type, body = desktop_api.route_request("GET", "/api/archival-mint-pagination-plan")
    ...
    def test_archival_account_state_provider_evaluation_payload_is_review_only(self):
        payload = desktop_api.build_archival_account_state_provider_evaluation_payload({
            "archival_account_state_provider_evaluation": {
                "mode": "ARCHIVAL_ACCOUNT_STATE_PROVIDER_EVALUATION_REVIEW_ONLY",
                "summary": {"candidate_providers": 1},
                "providers": [{"provider_id": "quicknode_solana_mainnet_archive", "status": "candidate_needs_manual_probe"}],
            }
        })

        self.assertEqual(payload["mode"], "ARCHIVAL_ACCOUNT_STATE_PROVIDER_EVALUATION_REVIEW_ONLY")
        self.assertTrue(payload["read_only"])
        self.assertTrue(payload["review_only"])
        self.assertTrue(payload["live_execution_locked"])
        self.assertFalse(payload["wallet_list_apply_allowed"])
    ...
    def test_archival_account_state_provider_evaluation_route_is_read_only(self):
        with mock.patch.object(desktop_api, "build_archival_account_state_provider_evaluation_payload", return_value={"mode": "ARCHIVAL_ACCOUNT_STATE_PROVIDER_EVALUATION_REVIEW_ONLY", "live_execution_locked": True}) as build_payload:
            status, content_type, body = desktop_api.route_request("GET", "/api/archival-account-state-provider-evaluation")
    ...
    def test_archival_account_state_provider_probe_payload_is_review_only(self):
        payload = desktop_api.build_archival_account_state_provider_probe_payload({
            "archival_account_state_provider_probe": {
                "mode": "ARCHIVAL_ACCOUNT_STATE_PROVIDER_PROBE_REVIEW_ONLY",
                "summary": {"probe_targets": 1},
                "probe_target": {"token_mint": "MintProbe111"},
            }
        })

        self.assertEqual(payload["mode"], "ARCHIVAL_ACCOUNT_STATE_PROVIDER_PROBE_REVIEW_ONLY")
        self.assertTrue(payload["read_only"])
        self.assertTrue(payload["review_only"])
        self.assertTrue(payload["live_execution_locked"])
        self.assertFalse(payload["wallet_list_apply_allowed"])
    ...
    def test_archival_account_state_provider_probe_route_is_read_only(self):
        with mock.patch.object(desktop_api, "build_archival_account_state_provider_probe_payload", return_value={"mode": "ARCHIVAL_ACCOUNT_STATE_PROVIDER_PROBE_REVIEW_ONLY", "live_execution_locked": True}) as build_payload:
            status, content_type, body = desktop_api.route_request("GET", "/api/archival-account-state-provider-probe")
    ...
    def test_archival_account_state_provider_probe_request_payload_is_review_only(self):
        payload = desktop_api.build_archival_account_state_provider_probe_request_payload({
            "archival_account_state_provider_probe_request": {
                "mode": "ARCHIVAL_ACCOUNT_STATE_PROVIDER_REQUEST_BUNDLE_REVIEW_ONLY",
                "summary": {"request_bundles_prepared": 1},
                "request_bundle": {"token_mint": "MintProbe111"},
            }
        })

        self.assertEqual(payload["mode"], "ARCHIVAL_ACCOUNT_STATE_PROVIDER_REQUEST_BUNDLE_REVIEW_ONLY")
        self.assertTrue(payload["read_only"])
        self.assertTrue(payload["review_only"])
        self.assertTrue(payload["live_execution_locked"])
    ...
    def test_archival_account_state_provider_probe_request_route_is_read_only(self):
        with mock.patch.object(desktop_api, "build_archival_account_state_provider_probe_request_payload", return_value={"mode": "ARCHIVAL_ACCOUNT_STATE_PROVIDER_REQUEST_BUNDLE_REVIEW_ONLY", "live_execution_locked": True}) as build_payload:
            status, content_type, body = desktop_api.route_request("GET", "/api/archival-account-state-provider-probe-request")
